/**
 * 错误码常量
 * 定义系统中使用的错误码
 * 采用分段编码体系：
 * 1000-1999: 通用错误
 * 2000-2999: 认证相关错误
 * 3000-3999: 用户相关错误
 * 4000-4999: 权限相关错误
 * 5000-5999: 角色相关错误
 * 6000-6999: 菜单相关错误
 * 9000-9999: 业务相关错误
 */

export const ERROR_CODES = {
  // 成功
  SUCCESS: 200,

  // 认证相关错误 (2000-2999)
  INVALID_TOKEN: 2001,
  ACCOUNT_LOCKED: 2004,
  ACCOUNT_DISABLED: 2005,
  AUTH_TOKEN_INVALID: 2009,
  AUTH_USER_INFO_MISSING: 2010,
  AUTH_INVALID_CREDENTIALS: 2011,
  AUTH_LOGIN_FAILED: 2012,
  AUTH_PASSWORD_MISMATCH: 2013,

  // 用户相关错误 (3000-3999)
  USER_NOT_FOUND: 3000,
  USER_ALREADY_EXISTS: 3001,
  USER_ID_UNDEFINED: 3011,
  INVALID_USER_ID: 3012,
  INVALID_PASSWORD: 3013,
  PASSWORD_SAME_AS_CURRENT: 3014,
  USER_INFO_RETRIEVAL_FAILED: 3015,

  // 权限相关错误 (4000-4999)
  PERMISSION_ROLE_INSUFFICIENT: 4005,
  PERMISSION_INSUFFICIENT: 4006,
  PERMISSION_ALREADY_EXISTS: 4007,
  PERMISSION_NOT_FOUND: 4008,

  // 角色相关错误 (5000-5999)
  ROLE_NOT_FOUND: 5000,

  // 菜单相关错误 (6000-6999)
  MENU_NOT_FOUND: 6000,

  // 操作日志相关错误 (8000-8999)
  OPERATION_LOG_NOT_FOUND: 8000,

  // 图片相关错误 (7000-7999)
  IMAGE_NOT_FOUND: 7000,
  IMAGE_UPLOAD_FAILED: 7001,
  IMAGE_INVALID_FORMAT: 7002,
  IMAGE_SIZE_EXCEEDED: 7003,
  IMAGE_SUPABASE_ERROR: 7004,

  // 订单相关错误 (10000-10999)
  ORDER_NOT_FOUND: 10000,
  ORDER_STATUS_INVALID: 10001,
  ORDER_CANNOT_SHIP: 10002,
  ORDER_CANNOT_CLOSE: 10003,
  ORDER_CANNOT_MODIFY_ADDRESS: 10004,
  ORDER_STOCK_INSUFFICIENT: 10005,

  // 产品相关错误 (11000-11999)
  PRODUCT_NOT_FOUND: 11000,
  PRODUCT_SAVE_FAILED: 11001,
  PRODUCT_SKU_CODE_CONFLICT: 11002,
  PRODUCT_TOO_MANY_MAIN_IMAGES: 11003,

  // 业务相关错误 (9000-9999)
  VALIDATION_INVALID_ID: 9006,
  VALIDATION_FAILED: 9007,
} as const;

/**
 * 错误消息映射
 */
export const ERROR_MESSAGES: { readonly [key: number]: string } = {
  [ERROR_CODES.SUCCESS]: '操作成功',

  // 认证相关
  [ERROR_CODES.INVALID_TOKEN]: '认证令牌无效',
  [ERROR_CODES.ACCOUNT_LOCKED]: '账户已被锁定，请联系管理员',
  [ERROR_CODES.ACCOUNT_DISABLED]: '账户已被禁用，请联系管理员',
  [ERROR_CODES.AUTH_TOKEN_INVALID]: '认证令牌无效或已过期',
  [ERROR_CODES.AUTH_USER_INFO_MISSING]: '用户认证信息缺失',
  [ERROR_CODES.AUTH_INVALID_CREDENTIALS]: '账号或密码错误',
  [ERROR_CODES.AUTH_LOGIN_FAILED]: '登录失败，请稍后重试',
  [ERROR_CODES.AUTH_PASSWORD_MISMATCH]: '账号或密码错误',

  // 用户相关
  [ERROR_CODES.USER_NOT_FOUND]: '用户不存在',
  [ERROR_CODES.USER_ALREADY_EXISTS]: '用户已存在',
  [ERROR_CODES.USER_ID_UNDEFINED]: '用户ID未定义',
  [ERROR_CODES.INVALID_USER_ID]: '无效的用户ID',
  [ERROR_CODES.INVALID_PASSWORD]: '无效的密码',
  [ERROR_CODES.PASSWORD_SAME_AS_CURRENT]: '新密码不能与当前密码相同',
  [ERROR_CODES.USER_INFO_RETRIEVAL_FAILED]: '获取用户信息失败',

  // 权限相关
  [ERROR_CODES.PERMISSION_ROLE_INSUFFICIENT]: '角色权限不足',
  [ERROR_CODES.PERMISSION_INSUFFICIENT]: '权限不足',
  [ERROR_CODES.PERMISSION_ALREADY_EXISTS]: '权限已存在',
  [ERROR_CODES.PERMISSION_NOT_FOUND]: '权限不存在',

  // 角色相关
  [ERROR_CODES.ROLE_NOT_FOUND]: '角色不存在',

  // 菜单相关
  [ERROR_CODES.MENU_NOT_FOUND]: '菜单不存在',

  // 操作日志相关
  [ERROR_CODES.OPERATION_LOG_NOT_FOUND]: '操作日志不存在',

  // 图片相关
  [ERROR_CODES.IMAGE_NOT_FOUND]: '图片不存在',
  [ERROR_CODES.IMAGE_UPLOAD_FAILED]: '图片上传失败',
  [ERROR_CODES.IMAGE_INVALID_FORMAT]: '图片格式不支持',
  [ERROR_CODES.IMAGE_SIZE_EXCEEDED]: '图片大小超出限制',
  [ERROR_CODES.IMAGE_SUPABASE_ERROR]: 'Supabase存储服务错误',

  // 订单相关
  [ERROR_CODES.ORDER_NOT_FOUND]: '订单不存在',
  [ERROR_CODES.ORDER_STATUS_INVALID]: '订单状态无效',
  [ERROR_CODES.ORDER_CANNOT_SHIP]: '订单无法发货',
  [ERROR_CODES.ORDER_CANNOT_CLOSE]: '订单无法关闭',
  [ERROR_CODES.ORDER_CANNOT_MODIFY_ADDRESS]: '订单无法修改地址',
  [ERROR_CODES.ORDER_STOCK_INSUFFICIENT]: '商品库存不足',

  // 产品相关
  [ERROR_CODES.PRODUCT_NOT_FOUND]: '商品不存在',
  [ERROR_CODES.PRODUCT_SAVE_FAILED]: '保存商品失败',
  [ERROR_CODES.PRODUCT_SKU_CODE_CONFLICT]: 'SKU编码已被其他商品使用',
  [ERROR_CODES.PRODUCT_TOO_MANY_MAIN_IMAGES]: '最多只能设置一张主图',

  // 业务相关
  [ERROR_CODES.VALIDATION_INVALID_ID]: 'ID验证失败',
  [ERROR_CODES.VALIDATION_FAILED]: '数据验证失败',
} as const;
