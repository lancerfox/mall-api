...
export class MenuService {
  ...
  async update(updateMenuDto: UpdateMenuDto): Promise<Menu> {
    ...
    let newPermissionModule = this.getModuleFromPath(updateData.path || menu.path);
    ...
    // 如果菜单已有权限，检查是否需要更新权限名称或模块
      ...
      // 检查权限名称或模块是否发生变化
      const currentPermission = await this.permissionService.findByName(menu.autoGeneratedPermission);
      ...
      else if (currentPermission.name !== newPermissionName || currentPermission.module !== newPermissionModule) {
        // 权限名称或模块发生变化，需要更新权限
        await this.permissionService.updateByName(menu.autoGeneratedPermission, {
          name: newPermissionName,
          description: `访问 ${updateData.name || menu.name} 页面的权限`,
          module: newPermissionModule,
        });
