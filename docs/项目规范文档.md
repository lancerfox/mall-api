...
为提高代码质量、可读性、可维护性，统一团队开发规范，特制定此文档。本文档涵盖了技术栈、目录结构、命名、代码风格、接口设计、状态码、Git 提交等方面的规范。
...
- **`modules`**: 存放核心业务模块，每个模块应有自己独立的目录，包含 `controller`, `service`, `module` 等文件。
...
### 8.1. 提交信息 (Commit Message)

遵循 **Conventional Commits** 规范，格式为：
...
- **type**:
  ...
  - `test`: 添加或修改测试
  - `chore`: 构建过程或辅助工具的变动
- **scope** (可选): 本次提交影响的范围，如 `user`, `auth`。
- **subject**: 简明扼要地描述本次提交的内容。

### 8.2. 示例

- `feat(auth): add JWT authentication strategy`
- `fix(user): correct user password update logic`
- `docs(readme): update project setup instructions`

### 8.3. 工具

推荐使用 `commitizen` 和 `commitlint` 辅助生成和校验规范的提交信息。